...
class NowPlayingMoviesRemoteMediator(
    ...
    override suspend fun load(
        ...
        val page = when (loadType) {
            LoadType.REFRESH -> {
                val remoteKeys =
                    getRemoteKeyClosestToCurrentPosition(state, MovieTypeTable.NowPlaying)
                remoteKeys?.nextKey?.minus(1) ?: START_INDEX
            }
            LoadType.PREPEND -> {
                val remoteKeys = getRemoteKeyForFirstItem(state, MovieTypeTable.NowPlaying)
                val prevKey = remoteKeys?.prevKey
                    ?: return MediatorResult.Success(endOfPaginationReached = remoteKeys != null)
                prevKey
            }
            LoadType.APPEND -> {
                val remoteKeys = getRemoteKeyForLastItem(state, MovieTypeTable.NowPlaying)
        ...
        try {
            return when (val result =
                movieRemoteDataSource.getMovies(page, MovieTypeParam.NowPlaying)) {
                ...
                is Result.Success -> {
                    ...
                    val keys = result.data.map {
                        RemoteKeyTable(
                            movieId = it.id,
                            type = MovieTypeTable.NowPlaying,
                            prevKey = prevKey,
                            nextKey = nextKey
                        )
                    }
