...
import br.com.popularmovies.repositories.mappers.toTopRatedTable
...
class TopRatedMoviesRemoteMediator(
...
) : RemoteMediator<Int, MovieTable.TopRated>() {

    override suspend fun load(
        loadType: LoadType,
        state: PagingState<Int, MovieTable.TopRated>
    ): MediatorResult {
        val page = when (loadType) {
            LoadType.REFRESH -> {
                val remoteKeys =
                    getRemoteKeyClosestToCurrentPosition(state, MovieTypeTable.TopRated)
                remoteKeys?.nextKey?.minus(1) ?: START_INDEX
            }
            LoadType.PREPEND -> {
                val remoteKeys = getRemoteKeyForFirstItem(state, MovieTypeTable.TopRated)
                val prevKey = remoteKeys?.prevKey
                    ?: return MediatorResult.Success(endOfPaginationReached = remoteKeys != null)
                prevKey
            }
            LoadType.APPEND -> {
                val remoteKeys = getRemoteKeyForLastItem(state, MovieTypeTable.TopRated)
        ...
        try {
            return when (val result =
                movieRemoteDataSource.getMovies(page, MovieTypeParam.TopRated)) {
                ...
                is Result.Success -> {
                    ...
                    if (loadType == LoadType.REFRESH) {
                        ...
                        movieLocalDataSource.deleteAllTopRatedMovies()
                    }
                    ...
                    val keys = result.data.map {
                        RemoteKeyTable(
                            movieId = it.id,
                            type = MovieTypeTable.TopRated,
                            prevKey = prevKey,
                            nextKey = nextKey
                        )
                    }
                    remoteKeyLocalDataSource.insertAll(keys)
                    movieLocalDataSource.insertAllTopRatedMovies(result.data.map { movie ->
                        movie.toTopRatedTable()
                    })
    ...
    private suspend fun getRemoteKeyClosestToCurrentPosition(
        state: PagingState<Int, MovieTable.TopRated>, type: MovieTypeTable
    ): RemoteKeyTable? {
